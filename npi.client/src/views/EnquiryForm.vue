  ...
  <div class="page-root d-flex flex-column">
    ...
    <div class="flex-grow-1 overflow-y-auto form-body pa-6">
      <v-container fluid class="pa-0 mx-auto" style="max-width: 1000px;">
          ...
          <v-card-text class="pa-4">
            <v-row dense>
                ...
                <v-autocomplete v-model="formData.cust_id"
                  ...
                  <template #item="{ props, item }">
                    <v-list-item v-bind="props">
                      <template #prepend>
                        <v-avatar color="primary" size="small"><v-icon>mdi-domain</v-icon></v-avatar>
                      </template>
              ...
              <v-col v-if="customerType === 'existing' && selectedCustomerInfo" cols="12">
                <v-card variant="tonal" color="primary" class="mt-2">
                  <v-card-text class="py-3 px-4 text-body-2">
                    <v-row dense>
                      <v-col cols="12" md="4"><strong>Contact:</strong> {{ selectedCustomerInfo.contact_name || 'N/A' }}</v-col>
                      <v-col cols="12" md="4"><strong>Email:</strong> {{ selectedCustomerInfo.contact_email || 'N/A' }}</v-col>
                      <v-col cols="12" md="4"><strong>Phone:</strong> {{ selectedCustomerInfo.contact_phone || 'N/A' }}</v-col>
                      <v-col cols="12"><strong>Address:</strong> {{ selectedCustomerInfo.cust_addr || 'N/A' }}</v-col>
                    </v-row>
                  </v-card-text>
                </v-card>
              </v-col>
              ...
              <template v-if="customerType === 'new'">
                ...
                <v-col cols="12" md="6">
                  <v-text-field v-model="formData.new_customer.contact_email" label="Email *" type="email"
                                prepend-inner-icon="mdi-email" density="compact" variant="outlined"
                                :rules="emailRules" />
                </v-col>
                <v-col cols="12" md="6">
                ...
                <v-col cols="12">
                  <v-textarea v-model="formData.new_customer.cust_addr" label="Address *"
                              prepend-inner-icon="mdi-map-marker" rows="2" density="compact" variant="outlined"
                              :rules="[v => !!v || 'Required']" />
                </v-col>
              </template>
            </v-row>
        ...
        <!-- NPI CATEGORY (Multi-Column Layout) -->
        <v-card class="mb-5 form-card" elevation="0" border>
          ...
          <v-card-text class="pa-4">
            <v-radio-group v-model="formData.npi_category" hide-details>
              <v-row dense>
                <v-col cols="12" sm="6" md="4" v-for="cat in configStore.categories" :key="cat.category_id">
                  <v-radio :label="cat.category_name" :value="cat.category_name" color="primary" density="compact" />
                </v-col>
              </v-row>
            </v-radio-group>
          </v-card-text>
        ...
        <!-- DYNAMIC SECTIONS -->
        <template v-for="(section, index) in activeSections" :key="section.section_id">
          <v-card class="mb-5 form-card" elevation="0" border>
            <v-card-title class="bg-grey-lighten-4 text-subtitle-1 font-weight-bold pa-3">
              {{ index + 2 }}. {{ section.section_label }}
            </v-card-title>
            <v-divider />
            <v-card-text class="pa-4">
              <v-row dense>
                <v-col v-for="field in section.fields" :key="field.field_id" cols="12" md="6">

                  <v-text-field v-if="field.field_type === 'text' || field.field_type === 'number'"
                                v-model="formData.dynamicFields[section.section_key][field.field_key]"
                                :label="field.field_label + (field.is_required ? ' *' : '')"
                                :type="field.field_type" density="compact" variant="outlined" hide-details="auto" class="mb-2"
                                :rules="field.is_required ? [v => !!v || 'Required'] : []" />

                  <v-text-field v-else-if="field.field_type === 'date'"
                                v-model="formData.dynamicFields[section.section_key][field.field_key]"
                                :label="field.field_label + (field.is_required ? ' *' : '')"
                                type="date" density="compact" variant="outlined" hide-details="auto" class="mb-2"
                                :rules="field.is_required ? [v => !!v || 'Required'] : []" />

                  <v-select v-else-if="field.field_type === 'select'"
                            v-model="formData.dynamicFields[section.section_key][field.field_key]"
                            :label="field.field_label + (field.is_required ? ' *' : '')"
                            :items="field.options ?? []" density="compact" variant="outlined" hide-details="auto" class="mb-2"
                            :rules="field.is_required ? [v => !!v || 'Required'] : []" />
                </v-col>
              </v-row>
            </v-card-text>
            ...
            <v-row v-if="uploadedFiles.length > 0" dense>
              <v-col cols="12">
                <v-list density="compact" class="bg-grey-lighten-4 rounded mt-2">
                  <v-list-subheader class="text-caption font-weight-bold">Previously Uploaded Files</v-list-subheader>
...
<script setup>
  ...
  import { api } from '@/utils/api' // Using api directly for file upload with params
  ...
  watch(() => configStore.sections, (sections) => {
    const df = {}
    sections.forEach(s => {
      df[s.section_key] = df[s.section_key] ?? {}
      s.fields.forEach(f => {
        df[s.section_key][f.field_key] = df[s.section_key][f.field_key] ?? ''
      })
    })
    Object.entries(df).forEach(([sk, fields]) => {
      if (!formData.value.dynamicFields[sk]) {
        formData.value.dynamicFields[sk] = fields
      } else {
        Object.keys(fields).forEach(fk => {
          if (formData.value.dynamicFields[sk][fk] === undefined)
            formData.value.dynamicFields[sk][fk] = ''
        })
      }
    })
  }, { immediate: true })
  ...
  watch(customerType, (newType) => {
    if (newType === 'new') {
      ...
      formData.value.new_customer = { comp_name: '', cust_addr: '', contact_name: '', contact_email: '', contact_phone: '' }
    }
  })

  function onCustomerSelect(custId) {
    selectedCustomerInfo.value = custId ? customers.value.find(c => c.cust_id === custId) ?? null : null
  }
    ...
    for (const section of activeSections.value) {
      for (const field of section.fields) {
        if (field.is_required) {
          const val = formData.value.dynamicFields[section.section_key]?.[field.field_key]
          if (!val) return false
        }
      }
    }
  ...
  function buildEnquiryData() {
    const data = { npi_category: formData.value.npi_category }
    ...
    const gf = formData.value.dynamicFields['generalInfo']
    if (gf && activeSections.value.some(s => s.section_key === 'generalInfo')) data.GeneralInfo = { ...gf }

    const sf = formData.value.dynamicFields['sealInfo']
    if (sf && activeSections.value.some(s => s.section_key === 'sealInfo')) data.SealInfo = { ...sf }

    data.CustomerRef = { mould_ownership: formData.value.customerRef.mould_ownership }
    return data
  }
  ...
  async function uploadFiles(enquiryId) {
    ...
    // Resolve the company name for the folder path
    ...
    const cleanCompName = encodeURIComponent(compName.replace(/[^a-zA-Z0-9 _-]/g, '').trim())

    // We bypass `enquiryStore.uploadFile` here to specifically inject the `comp_name` query param
    for (const file of selectedFiles.value) {
      const fileData = new FormData()
      fileData.append('file', file)
      try {
        await api.post(`/enquiry/${enquiryId}/upload?comp_name=${cleanCompName}`, fileData, {
  ...
  async function saveDraft() {
    if (!validateCustomer()) { errorMessage.value = 'Please select or enter customer information'; return }
    saving.value = true; errorMessage.value = ''; successMessage.value = ''
    try {
      const result = isEdit.value
        ? await enquiryStore.updateEnquiry(route.params.id, buildEnquiryData())
        : await enquiryStore.createEnquiry(buildEnquiryData())

      if (result.success) {
        successMessage.value = 'Draft saved successfully!'
        if (selectedFiles.value.length > 0) {
          await uploadFiles(isEdit.value ? route.params.id : result.data.enquiry.enquiry_id)
        }
  ...
  async function handleSubmit() {
    if (!validateCustomer()) { errorMessage.value = 'Please select or enter customer information'; return }
    if (!validateForm()) { errorMessage.value = 'Please fill in all required fields'; return }
    saving.value = true; errorMessage.value = ''; successMessage.value = ''
    try {
      ...
      if (result.success) {
        const eId = isEdit.value ? route.params.id : result.data.enquiry.enquiry_id
        if (selectedFiles.value.length > 0) await uploadFiles(eId)
        result = await enquiryStore.submitEnquiry(eId)
      }
  ...
  onMounted(async () => {
    ...
    if (route.params.id) {
      ...
      if (result?.success && result.data) {
        ...
        if (enq.generalInfo) formData.value.dynamicFields['generalInfo'] = { ...enq.generalInfo }
        if (enq.sealInfo) formData.value.dynamicFields['sealInfo'] = { ...enq.sealInfo }
        if (enq.customerRef) formData.value.customerRef.mould_ownership = enq.customerRef.mould_ownership ?? ''
...
<style scoped>
  ...
  /* Enable internal scrolling on the form body only */
  .form-body {
    overflow-y: auto;
  }
