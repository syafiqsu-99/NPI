  ...
  <v-container fluid class="pa-4">
    ...
    <v-row class="mt-4">
      <v-col cols="12" v-for="project in projectsWithTasks" :key="project.proj_id">
        <v-card elevation="2">
          <!-- Project Header -->
          <v-card-title class="bg-grey-lighten-3 d-flex align-center justify-space-between">
                ...
                <v-menu>
                  ...
                  <v-list density="compact">
                    <v-list-item v-for="status in statusOptions"
                      ...
                      <v-list-item-title>
                        <v-icon start size="small" :color="getStatusColor(status)">
                  ...
                  <v-tooltip text="Upload Documents" location="top">
                    <template #activator="{ props }">
                      <v-btn icon="mdi-file-upload"
                             ...
                             color="primary"
                             v-bind="props"
                             @click="openUploadDialog(item)"></v-btn>
                    </template>
                  </v-tooltip>

                  <v-tooltip text="View Documents" location="top">
                    <template #activator="{ props }">
                      <v-badge :content="getDocumentCount(item)"
                               :model-value="getDocumentCount(item) > 0"
                               color="success">
                        <v-btn icon="mdi-file-document-multiple"
                               ...
                               color="info"
                               v-bind="props"
                               @click="openDocumentsDialog(item)"></v-btn>
                      </v-badge>
                    </template>
                  </v-tooltip>
                  ...
                  <v-tooltip text="Task Details" location="top">
                    <template #activator="{ props }">
                      <v-btn icon="mdi-eye"
                             ...
                             variant="text"
                             v-bind="props"
                             @click="openTaskDetailsDialog(item)"></v-btn>
                    </template>
                  </v-tooltip>
                </div>
              </template>
      ...
      <v-col cols="12" v-if="projectsWithTasks.length === 0 && !loading">
        <v-card>
          <v-card-text class="text-center pa-8">
            ...
            <div class="text-h6 mt-4 text-grey">No tasks assigned to your department</div>
            <div class="text-caption text-grey">
              Tasks will appear here once they are assigned to your department
            </div>
          </v-card-text>
        </v-card>
      </v-col>
      ...
      <v-card>
        ...
        <v-card-text class="pt-4">
          ...
          <v-alert type="info" variant="tonal" class="mt-4">
            <div class="text-caption">
              <strong>Allowed file types:</strong> PDF, Word, Excel, Images (PNG, JPG)
            </div>
            <div class="text-caption">
    ...
    <v-dialog v-model="documentsDialog" max-width="800px">
      <v-card>
        <v-card-title class="bg-primary d-flex align-center justify-space-between">
          <div>
          ...
          <v-list v-if="taskDocuments.length > 0">
            <v-list-item v-for="doc in taskDocuments" :key="doc.file_id" class="border-b">
              ...
              <v-list-item-subtitle>
                ...
                <div v-if="doc.description" class="text-caption text-grey mt-1">
                  {{ doc.description }}
                </div>
              </v-list-item-subtitle>
              ...
              <template #append>
                <div class="d-flex ga-2">
                  <v-btn icon="mdi-download"
                         size="small"
                         variant="text"
                         @click="downloadDocument(doc)"></v-btn>
          ...
          <v-row>
            <v-col cols="12" md="6">
              <v-card variant="outlined">
                ...
                <v-card-text>
                  <div class="mb-2">
                    <strong>Project:</strong> {{ getProjectName(selectedTask.proj_id) }}
                  </div>
                  <div class="mb-2">
                  ...
                  <div class="mb-2">
                    ...
                    <v-chip :color="getPriorityColor(selectedTask.priority)" size="small" class="ml-2">
                      {{ selectedTask.priority }}
                    </v-chip>
                  </div>
                  <div class="mb-2">
                    <strong>Department:</strong> {{ selectedTask.dept_name || 'N/A' }}
                  </div>
                  <div class="mb-2">
                    <strong>Duration:</strong> {{ selectedTask.duration || 0 }} days
                  </div>
                  <div class="mb-2">
...
<script setup>
  ...
  // Table headers
  const headers = [
    { title: 'Task', key: 'title', width: '30%' },
    { title: 'Status', key: 'status', width: '15%' },
    { title: 'Priority', key: 'priority', width: '12%' },
    { title: 'Department', key: 'dept_name', width: '15%' },
    { title: 'Planned Dates', key: 'planned_dates', width: '18%' },
  ...
  const userDeptId = computed(() => authStore.currentUser?.dept_id)

  const filteredTasks = computed(() => { if (isAdmin.value) { return tasks.value } return tasks.value.filter(t => t.dept_id === userDeptId.value) })

  const projectsWithTasks = computed(() => {
    const projectMap = new Map()

    filteredTasks.value.forEach(task => {
      if (!projectMap.has(task.proj_id)) {
        const project = projects.value.find(p => p.proj_id === task.proj_id)
        if (project) {
          projectMap.set(task.proj_id, {
            ...project,
            tasks: []
          })
        }
      ...
      const projectData = projectMap.get(task.proj_id)
      if (projectData) {
        projectData.tasks.push(task)
      }
    })

    return Array.from(projectMap.values()).sort((a, b) => a.proj_name.localeCompare(b.proj_name))
  })

  const taskStats = computed(() => {
    const total = filteredTasks.value.length
    const inProgress = filteredTasks.value.filter(t => t.status === 'In Progress').length
    const completed = filteredTasks.value.filter(t => t.status === 'Completed').length
    const overdue = filteredTasks.value.filter(t => isOverdue(t)).length
  ...
  function getProjectStatusColor(project) {
    ...
    return colors[project.status] || 'grey'
  }
  ...
  function formatDateTime(datetime) {
    ...
    return new Date(datetime).toLocaleString('en-GB', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })
  }
  ...
  function getProjectName(projId) {
    const project = projects.value.find(p => p.proj_id === projId)
    return project?.proj_name || 'Unknown'
  }
  ...
  function getDocumentCount(task) {
    return task.document_count || 0
  }
  ...
  async function updateTaskStatus(task, newStatus) {
    try {
      ...
      if (result?.success) {
        ...
        snackbarMessage.value = 'Task status updated successfully'
  ...
  async function uploadDocuments() {
    ...
    try {
      ...
      if (result?.success) {
        snackbarMessage.value = 'Documents uploaded successfully'
        snackbarColor.value = 'success'
        snackbar.value = true
        closeUploadDialog()
        ...
        if (selectedTask.value) {
          selectedTask.value.document_count = (selectedTask.value.document_count || 0) + uploadFiles.value.length
        }
      }
    } catch (error) {
      console.error('Error uploading documents:', error)
      snackbarMessage.value = 'Failed to upload documents'
      snackbarColor.value = 'error'
      snackbar.value = true
    } finally {
  ...
  async function openDocumentsDialog(task) {
    selectedTask.value = task
  ...
  function openUploadFromDocuments() {
    ...
    openUploadDialog(selectedTask.value)
  }
  ...
  async function loadTaskDocuments(taskId) {
    try {
      ...
      if (result?.success && result?.data) {
        taskDocuments.value = result.data
      } else if (Array.isArray(result)) {
        taskDocuments.value = result
      }
    } catch (error) {
  ...
  async function deleteDocument() {
    ...
    try {
      ...
      if (result?.success) {
        ...
        snackbarMessage.value = 'Document deleted successfully'
        snackbarColor.value = 'success'
        snackbar.value = true
      ...
      snackbarMessage.value = 'Failed to delete document'
      snackbarColor.value = 'error'
      snackbar.value = true
    } finally {
  ...
  async function refreshTasks() {
    await loadTasks()
    snackbarMessage.value = 'Tasks refreshed'
